...
def get_admin_site_settings(
    ...
    settings = db.query(WebsiteSetting).filter(WebsiteSetting.key.in_(_ALLOWED_SITE_SETTING_KEYS)).all()
    return {s.key: s.value for s in settings}


# -------------------------------------------------------------
# Generic Section Image Upload (e.g. Natural Goodness showcase image)
# -------------------------------------------------------------
HOW_TO_USE_DIR = UPLOAD_ROOT / "how_to_use"
HOW_TO_USE_DIR.mkdir(parents=True, exist_ok=True)

SECTIONS_DIR = UPLOAD_ROOT / "sections"
SECTIONS_DIR.mkdir(parents=True, exist_ok=True)


@router.post(
    "/landing-page/sections/{section_key}/image",
    summary="Upload a generic image for any landing-page section (e.g. natural_goodness)",
)
async def upload_section_image(
    ...
    """
    Generic image upload for a landing-page section's draft content.
    Stores the uploaded image URL under draft_content["image_url"].
    Admin must click Publish to make it live on the customer site.
    """
    filename, rel_url, file_size = await _save_upload_file(
        file, SECTIONS_DIR, f"{section_key}_image"
    )
    ...
    media = MediaAsset(
        ...
        file_path=rel_url if is_remote else str(SECTIONS_DIR / filename),
        file_url=rel_url,
        mime_type=file.content_type or "image/jpeg",
        file_size=file_size,
        asset_type="section",
    ...
    db.commit()
    audit_service.log_action(
        db, "SECTION_IMAGE_UPLOADED", admin.id, "landing_page_section",
        section_key, {"url": rel_url}
    )
    db.commit()

    return {
        "success": True,
        "message": "Image uploaded as draft. Click Publish to make it live.",
        "image_url": rel_url,
        "section_key": section_key,
    }
