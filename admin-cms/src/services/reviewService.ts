...
/**
 * Admin: Fetch Google Reviews via Places API.
 */
export async function fetchGoogleReviews(placeId: string, apiKey?: string): Promise<{ success: boolean; message: string; added: number }> {
  const response = await apiClient.post("/api/v1/admin/reviews/fetch-google", { place_id: placeId, api_key: apiKey });
  return response.data;
}
